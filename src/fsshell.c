...
// UPDATED for fs_getcmd and fs_setcmd
...
int cmd_cd (int argcnt, char *argvec[]) {
    ...
    if (path[0] == '"' && path[strlen(path)-1] == '"')
    {
        ...
        memmove(path, path+1, strlen(path)-2);
...
#endif		
	cmdv[cmdc] = 0;	// Just to be safe - null terminate array of arguments
...
int main (int argc, char * argv[]) {
    ...
    printf(C_TITLE "+ Supported Commands\n" C_RESET);
    printf("  ls    cd    md   pwd\n");
    printf("  touch cat   rm   cp\n");
    printf("  mv    cp2fs cp2l help\n\n");

    printf("Type " C_PROMPT "`exit`" C_RESET " to quit\n\n");

	while (1) {
		cmdin = readline(C_PROMPT "Prompt > " C_RESET);
#ifdef COMMAND_DEBUG
		printf ("%s\n", cmdin);
#endif
